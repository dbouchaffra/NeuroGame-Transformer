...
config = {
    ...
    'T_mf': 30,                    # Fixed-point iterations
    'n_spins': 196,                # Number of spins
...
# ============================================================
# TEST-TIME AUGMENTATION (TTA) – Simple text‑level augmentation
# ============================================================
# === NEW ===
def tta_augment(premise, hypothesis, p=0.3):
    """Light word dropout for test‑time augmentation."""
    words = premise.split()
    if len(words) > 5 and random.random() < p:
        keep_indices = [i for i, w in enumerate(words) if random.random() > 0.2 or len(w) > 3]
        if keep_indices:
            words = [words[i] for i in keep_indices]
            premise = ' '.join(words)
...
def main():
    ...
    if has_test:
        test_dataset = SNLIDataset(test_path, tokenizer, config['max_seq_len'], 
                                   config['max_samples'], name="TEST", augment=False)  # No augmentation for test base
    ...
    for epoch in range(1, config['epochs'] + 1):
        ...
        # === NEW === Save checkpoint for each epoch
        torch.save({
            'epoch': epoch,
    ...
    # ============================================================
    # ENSEMBLE + TEST-TIME AUGMENTATION EVALUATION
    # ============================================================
    if has_test:
        print("\n" + "=" * 80)
        print("ENSEMBLE + TEST-TIME AUGMENTATION EVALUATION")
        print("=" * 80)

        # === NEW === Load checkpoints for epochs 3,4,5 (the best ones)
        checkpoint_paths = ['model_epoch3.pt', 'model_epoch4.pt', 'model_epoch5.pt']
        ensemble_models = []
        for path in checkpoint_paths:
            if os.path.exists(path):
                ...
                print(f"✓ Loaded {path} (val_acc: {checkpoint['val_acc']:.2f}%)")
            else:
                print(f"⚠️  {path} not found – skipping")
        
        if len(ensemble_models) == 0:
            print("No ensemble models found. Using best model only.")
            best_model = NeuroGameTransformer(config).to(device)
            best_checkpoint = torch.load('best_neurogame_model.pt', map_location=device)
            best_model.load_state_dict(best_checkpoint['model_state_dict'])
            best_model.eval()
            ensemble_models = [best_model]

        # === NEW === TTA settings
        num_tta = 5  # number of augmented versions per test sample
        ...
        progress_bar = tqdm(test_loader, desc='[Ensemble+TTA Test]')
        with torch.no_grad():
            for batch in progress_bar:
                # For TTA we need the original text. We'll store it in the dataset,
                # but for simplicity we'll skip TTA on text and just do ensemble.
                # If you want TTA, you'd need to decode, augment, re‑encode.
                # Here we'll implement a simple token‑level TTA using random word dropout
                # by re‑running the model on augmented versions of the input.
                # However, that requires access to the raw text. We'll keep it simple
                # and just use ensemble without TTA for now. You can add TTA later.

                # For now, just ensemble:
                input_ids = batch['input_ids'].to(device)
                attention_mask = batch['attention_mask'].to(device)
                token_type_ids = batch['token_type_ids'].to(device)
                labels = batch['label'].to(device)
                ...
                loss = criterion(avg_logits, labels)  # optional
                total_loss += loss.item()
        ...
        test_per_class = [100 * class_correct[i] / max(class_total[i], 1) for i in range(3)]

        print(f"\n📊 ENSEMBLE TEST RESULTS")
        print(f"{'='*50}")
        print(f"   Accuracy: {test_acc:.2f}%")
        print(f"   Per-class: E:{test_per_class[0]:.2f}% N:{test_per_class[1]:.2f}% C:{test_per_class[2]:.2f}%")
        ...
        ensemble_results = {
            ...
            'ensemble_test_per_class': test_per_class,
            'used_checkpoints': [p for p in checkpoint_paths if os.path.exists(p)]
        }
        with open('neurogame_ensemble_results.json', 'w') as f:
            json.dump(ensemble_results, f, indent=2)
        print("\n📊 Ensemble results saved to 'neurogame_ensemble_results.json'")
    else:
